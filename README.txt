READ-ME

For users, here are some pre-conditions to use the application:
1. Accept location permission, if denied accidently please refresh application once more and permissions access will prompt once again
2. LiveLocation marker: requires user to wait 5-30 seconds to load, or move around 5 footsteps in a location with good reception
3. Android device version: at least version 10 and above
4. Android devices preferred for optimal usage: Samsung


For developers:
1. Google API key: update API key credentials with your project's unique SHA-1 value in res>values>google_maps_api.xml file.
Refer to https://developers.google.com/codelabs/maps-platform/location-places-android#10